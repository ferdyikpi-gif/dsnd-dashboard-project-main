...
class ReportDropdown(Dropdown):
    ...
    def component_data(self, entity_id, model):
        ...
        # names and ids
        data = model.names()
        # The parent dropdown expects (value, label) tuples,
        # but our model.names() returns (name, id). Swap them.
        return [(id_val, name_val) for name_val, id_val in data]
...
class BarChart(MatplotlibViz):
    ...
    def visualization(self, data, **kwargs):
        ...
        if model and getattr(model, 'name', None) == "team":
            ...
            pred = probs[0] if len(probs) > 0 else 0
